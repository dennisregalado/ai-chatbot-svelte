...
import type { ChatInit, ChatTransport, UIMessage as Message, UIMessage } from 'ai';
...
type UseChatParams<M extends UIMessage = UIMessage> = ChatInit<M> & UseChatOptions<M>;

/**
 * Options for the useAgentChat hook
 */
type UseAgentChatOptions<State, ChatMessage extends UIMessage = UIMessage> = Omit<
...
export function useAgentChat<State = unknown, ChatMessage extends UIMessage = UIMessage>(
	...
	const aiFetch = useCallback(async (request: RequestInfo | URL, options: RequestInit = {}) => {
		const {
			method,
			keepalive,
			headers,
	...
	const addToolResultAndSendMessage: typeof useChatHelpers.addToolResult = async (args) => {
	...
	return {
		...
		setMessages: (messages: Parameters<typeof useChatHelpers.setMessages>[0]) => {
