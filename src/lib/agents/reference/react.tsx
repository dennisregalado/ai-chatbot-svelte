...
import type { Agent, MCPServersState, RPCRequest, RPCResponse } from 'agents';
...
function setCacheEntry(key: unknown[], value: Promise<QueryObject>, cacheTtl?: number): void {
	...
	const expiresAt = cacheTtl ? Date.now() + cacheTtl : Date.now() + 5 * 60 * 1000; // Default 5 minutes
...
export function useAgent<State>(options: UseAgentOptions<unknown>): PartySocket & {
	agent: string;
	name: string;
	setState: (state: State) => void;
