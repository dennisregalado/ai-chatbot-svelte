...
export const createUser = command(z.object({
    email: z.string(),
    password: z.string(),
...
export const saveChat = command(z.object({
    id: z.string(),
    userId: z.string(),
    title: z.string(),
    visibility: z.enum(['public', 'private']),
}), async ({ id, userId, title, visibility }: { id: string; userId: string; title: string; visibility: VisibilityType }) => {
...
export const getChatsByUserId = query(z.object({
    id: z.string(),
    limit: z.number(),
    startingAfter: z.string().nullable(),
    endingBefore: z.string().nullable(),
}), async ({ id, limit, startingAfter, endingBefore }: { id: string; limit: number; startingAfter: string | null; endingBefore: string | null }) => {
...
export const saveMessages = command(z.object({
...
}), async ({ messages }: { messages: Array<{ id: string; chatId: string; role: string; parts: unknown; attachments: unknown; createdAt: Date }> }) => {
...
export const voteMessage = command(z.object({
    chatId: z.string(),
    messageId: z.string(),
    type: z.enum(['up', 'down']),
}), async ({ chatId, messageId, type }: { chatId: string; messageId: string; type: 'up' | 'down' }) => {
...
export const saveDocument = command(z.object({
    id: z.string(),
    title: z.string(),
    kind: z.enum(['text', 'code', 'image', 'sheet']),
    content: z.string(),
    userId: z.string(),
}), async ({ id, title, kind, content, userId }: { id: string; title: string; kind: 'text' | 'code' | 'image' | 'sheet'; content: string; userId: string }) => {
...
export const deleteDocumentsByIdAfterTimestamp = command(z.object({
    id: z.string(),
    timestamp: z.date(),
...
export const saveSuggestions = command(z.object({
...
}), async ({ suggestions }: { suggestions: Array<{ id: string; documentId: string; documentCreatedAt: Date; originalText: string; suggestedText: string; description: string | null; isResolved: boolean; userId: string; createdAt: Date }> }) => {
...
export const deleteMessagesByChatIdAfterTimestamp = command(z.object({
    chatId: z.string(),
    timestamp: z.date(),
...
export const updateChatVisiblityById = command(z.object({
    chatId: z.string(),
...
export const getMessageCountByUserId = query(z.object({
    id: z.string(),
...
export const createStreamId = command(z.object({
    streamId: z.string(),
    chatId: z.string(),
